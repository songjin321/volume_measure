...
vector<Eigen::Vector3d> Elas::process (uint8_t* I1_,uint8_t* I2_,float* D1,float* D2,const int32_t* dims){
...
//	 res.push_back(Eigen::Vector3d(p_support[i].u,p_support[i].v,p_support[i].d));
//  }
...
//  std::cout << "support point : " << p_support.size() << endl;
#ifdef PROFILE
  timer.start("Delaunay Triangulation");
#endif
  std::vector<triangle> tri_1 = computeDelaunayTriangulation(p_support,0);
  std::vector<triangle> tri_2 = computeDelaunayTriangulation(p_support,1);

  for(int i=0; i< tri_1.size();i++)
  {
	  res.push_back(Eigen::Vector3d(p_support[tri_1[i].c1].u,p_support[tri_1[i].c1].v,p_support[tri_1[i].c1].d));
	  res.push_back(Eigen::Vector3d(p_support[tri_1[i].c2].u,p_support[tri_1[i].c2].v,p_support[tri_1[i].c2].d));
	  res.push_back(Eigen::Vector3d(p_support[tri_1[i].c3].u,p_support[tri_1[i].c3].v,p_support[tri_1[i].c3].d));
  }
